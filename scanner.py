...
font_size_base = 12  # Taille par défaut

header = tk.Label(root, text="Scanner de Code-Barre", font=("Helvetica", 18, "bold"), fg="#ecf0f1", bg="#2c3e50")
header.pack(pady=10)

def bind_shortcuts_to_window(window):
    """
    Ajoute les raccourcis de zoom à une fenêtre donnée.
    Permet d'agrandir ou de réduire dynamiquement les textes et widgets.
    :param window: Fenêtre (Tk ou Toplevel) cible pour ajouter les bindings.
    """
    # Binding clavier pour agrandir/réduire
    window.bind("<Control-Key-=>", lambda event: increase_font_size_in_window(window))  # Ctrl + +
    window.bind("<Control-Key-minus>", lambda event: decrease_font_size_in_window(window))  # Ctrl + -

    # Zoom avec la molette
    window.bind("<Control-MouseWheel>", lambda event: adjust_with_mouse_in_window(window, event))


def increase_font_size_in_window(window):
    """
    Augmente la taille des polices pour une fenêtre spécifique.
    """
    adjust_font_size_and_window(1.2, window)  # Augmenter de 20%


def decrease_font_size_in_window(window):
    """
    Diminue la taille des polices pour une fenêtre spécifique.
    """
    adjust_font_size_and_window(0.8, window)  # Réducer de 20%


def adjust_with_mouse_in_window(window, event):
    """
    Ajuste la taille des polices avec la molette dans une fenêtre donnée.
    """
    if event.delta > 0:  # Molette vers le haut
        increase_font_size_in_window(window)
    else:  # Molette vers le bas
        decrease_font_size_in_window(window)

def adjust_font_size_and_window(scale_factor, window):
    """
    Ajuste la taille des polices et des widgets d'une fenêtre spécifique.
    :param scale_factor: Facteur d'agrandissement ou de réduction des polices/widgets.
    :param window: Fenêtre (root ou Toplevel) pour ajuster les widgets.
    """
    global font_size_base

    # Mettre à jour la base des polices
    font_size_base = int(font_size_base * scale_factor)

    # Appliquer le redimensionnement sur la fenêtre
    def update_widget_font(widget):
        if isinstance(widget, tk.Label) or isinstance(widget, tk.Button):
            current_font = widget["font"]
            font_name, font_size, *font_weight = current_font.split()
            new_font_size = int(font_size_base)
            widget.config(font=(font_name, new_font_size, *font_weight))
        elif isinstance(widget, tk.Entry):
            widget.config(font=("Helvetica", font_size_base))

        # Ajuster récursivement si le widget contient d'autres widgets (par exemple, les Frames)
        if widget.winfo_children():
            for child in widget.winfo_children():
                update_widget_font(child)

    # Appliquer cette mise à jour pour chaque widget de la fenêtre donnée
    for widget in window.winfo_children():
        update_widget_font(widget)

    # Ajuster la taille de la fenêtre (facultatif, commenté si fixe)
...
def open_all_articles_window():
    ...
    for code, product in data.items():
        ...
        tk.Label(frame, text=f"Type: {product_type}", font=("Helvetica", 10), bg="#2c3e50", fg="#ecf0f1").pack(
            anchor="w", padx=10)
        tk.Label(frame, text=f"nb: {count}", font=("Helvetica", 10), bg="#2c3e50", fg="#ecf0f1").pack(
            anchor="w", padx=10)
        ...
        if image_url:
            try:
                ...
                tk.Label(frame, text="Image non disponible", font=("Helvetica", 8), bg="#2c3e50", fg="#e74c3c").pack(
                    anchor="w", padx=10)
    bind_shortcuts_to_window(articles_window)
...
def open_statistics_window(event=None):
    data = load_existing_data()
    total_count = sum(product.get("nb", 0) for product in data.values())
    unique_codes = len(data)
    average_count = total_count / unique_codes if unique_codes > 0 else 0
    top_n = sorted(data.items(), key=lambda x: x[1].get("nb", 0), reverse=True)[:5]
    ...
    tk.Label(stats_window, text="Statistiques des Produits", font=("Helvetica", 14, "bold"), bg="#34495e",
             fg="#ecf0f1").pack(pady=10)
    tk.Label(stats_window, text=f"nb Total de Produits : {total_count}", font=("Helvetica", 12), bg="#34495e",
             fg="#ecf0f1").pack(
        pady=5)
    tk.Label(stats_window, text=f"Codes-Barres Uniques : {unique_codes}", font=("Helvetica", 12), bg="#34495e",
             fg="#ecf0f1").pack(
        pady=5)
    tk.Label(stats_window, text=f"nb Moyen par Produit : {average_count:.2f}", font=("Helvetica", 12),
             bg="#34495e", fg="#ecf0f1").pack(
        pady=5)

    # Calculate percentage contribution of top 5 products
    top_5_total = sum(x[1].get("nb", 0) for x in top_n)
    ...
    if data:
        least_scanned_product = min(data.items(), key=lambda x: x[1].get("nb", float('inf')))
        least_scanned_details = f"{least_scanned_product[0]}: {least_scanned_product[1].get('nom', 'Inconnu')} - {least_scanned_product[1].get('nb', 0)} fois"
    else:
    ...
    for code, product in top_n:
        tk.Label(stats_window, text=f"{code}: {product.get('nom', 'Inconnu')} - {product.get('nb', 0)} fois",
    ...
    bind_shortcuts_to_window(stats_window)
...
def modify_data(data, fetched_data=None):
    if fetched_data:
        ...
        data[input_value] = {
            ...
            "nb": data.get(input_value, {}).get("nb", 0) + 1
        }
        update_label_output(f"Produit ajouté : {input_value}\nNom: {data[input_value].get('nom', 'Inconnu')}, "
                            ...
                            f"nb: {data[input_value].get('nb', 0)}")
    else:
        data[input_value]["nb"] += 1
...
def reset_modes():
    global modifmode, retirermode, resetmode, copymode, displayinfomode
    modifmode = False
    retirermode = False
    resetmode = False
    copymode = False
...
def process_input(event=None):
    ...
    if retirermode:
        if input_value in data:
            if data[input_value]["nb"] >= 1:
                data[input_value]["nb"] -= 1
                update_label_output(
                    f"Produit retiré : {input_value}\nnb après retrait : {data[input_value]['nb']}")
            else:
                update_label_output(f"Produit non retirable: {input_value} (nb tombé à 0)")
            save_data(data)
        else:
...
def open_manual_entry_window():
    ...
    def save(event=None):
        ...
        description = f"Nom: {entry_data.get('nom', 'Inconnu')}, Marque: {entry_data.get('marque', 'Inconnu')}, " \
                      f"Quantité: {entry_data.get('quantite', 'Inconnu')}, nb: {entry_data.get('nb', 1)}"
    ...
    bind_shortcuts_to_window(manual_window)
...
def enter_ajout_mode(event=None):
    global modifmode
    reset_modes()
    update_label_output(f"Mode Ajout activé")
    reset_input()
...
def open_help_window():
    ...
    help_text = (
        ...
        "- Aggrandir la taille: 'Ctrl' + '+' ou bien 'Ctrl' + 'Molette Vers le Haut'\n"
        "- Rétrécir la taille: 'Ctrl' + '-' ou bien 'Ctrl' + 'Molette Vers le Bas'\n"
    ...
    bind_shortcuts_to_window(help_window)
